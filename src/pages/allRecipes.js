...
function renderCategoryChips(counts = {}) {
  ...
  const categoryChipsMarkup = RECIPE_CATEGORIES
    ...
    .map((category) => {
      ...
      const query = params.toString();
      const href = `./all-recipes.html${query ? `?${query}` : ''}`;
...
async function applyFavoriteAfterLoginIntent() {
  if (!currentUser || !favoriteAfterLoginRecipeId) {
    return;
  }

  try {
    const alreadyFavorited = await isRecipeFavorited(favoriteAfterLoginRecipeId, currentUser.id);

    if (!alreadyFavorited) {
      const nextState = await toggleRecipeFavorite(favoriteAfterLoginRecipeId, currentUser.id, false);
...
async function handleFavoriteButtonClick(button) {
  ...
  if (!currentUser) {
    const params = new URLSearchParams(window.location.search);
    params.set('favoriteAfterLogin', recipeId);
